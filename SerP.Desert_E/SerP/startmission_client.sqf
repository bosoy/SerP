...
if (!isnil "SerP_loading" && {SerP_loading==0} && {time<60} && {(player != leader group player)} && {!(serverCommandAvailable "#kick")} && {120>({isPlayer _x} count playableUnits)}) exitWith {
	failMission "loser";
};
if (!isnil "SerP_loading" && {SerP_loading==2} && {time<60} && {!((player == leader group player) || (serverCommandAvailable "#kick"))}) exitWith {
	failMission "loser";
};
_veh = (vehicle player);
openMap [true,true];
