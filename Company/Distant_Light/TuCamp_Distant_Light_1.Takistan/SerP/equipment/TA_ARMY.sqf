...
//МСВ
		...
		case "AT" : {//1й номер расчета РПГ
			{_unit addMagazine "30Rnd_545x39_AK"} forEach [0,0,0,0,0,0];
			_unit addWeapon "AK_74";
			{_unit addMagazine "ACE_PG7V_PGO7"} forEach [0,0,0];
		...
		case "AAT" : {//2й номер расчета РПГ
			...
			_unit addWeapon "ACE_Earplugs";
			_unit setVariable ["ACE_weapononback","ACE_ALICE_Backpack"];
			_unit setVariable ["ACE_RuckMagContents",[["ACE_PG7V_PGO7",4]]];
		};
...
//разведотделение
		...
		case "R_VSS" : {// Разведчик - ВСС
			{_unit addMagazine "20Rnd_9x39_SP5_VSS"} forEach [0,0,0,0,0,0,0,0];
...
//отделение сбора и эвакуации раненых
		case "MED_SL" : {//Командир отделения - санитарный инструктор
			...
			_unit addWeapon "NVGoggles";
			_unit addWeapon "ACE_CharliePack_ACU_Medic";
